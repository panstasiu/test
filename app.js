...
document.addEventListener("click",e=>{const btn=e.target.closest(".add-product");if(!btn)return;const card=btn.closest(".product-card");if(!card)return;const id=card.dataset.id;if(!id)return;const existing=state.cart.find(p=>p.id===id);if(existing){existing.qty=(existing.qty||1)+1}else{state.cart.push({id,name:card.dataset.name||"",price:Number(card.dataset.price)||0,img:card.querySelector("img")?.src||"",qty:1})}save();toast("Dodano do torby ✓");btn.textContent="Dodano ✓";clearTimeout(btn._cartTimer);btn._cartTimer=setTimeout(()=>btn.textContent="Dodaj do torby",1000)});
